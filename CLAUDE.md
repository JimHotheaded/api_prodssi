...
- **RMM1 logs at 15s since 2026-07-09 ~09:18 plant time** (Kepware interval changed as a test; was 10s). Its `countRMM1` defaults are `pointsPerHour=240` and gap-fill `cadence=15` (the RMM1 window route's opt-in fill also defaults to cadence 15). RMM1 queries on windows **before** the changeover need `&pointsPerHour=360&cadence=10`; windows spanning the changeover are inaccurate with any single divisor. If the interval test is rolled back to 10s, revert these RMM1 defaults.
